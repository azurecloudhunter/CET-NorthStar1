    ...
    Describe "E2E Integration Test for Tenant Deployment" {
        ...
        BeforeAll {
            ...
            if (Get-AzManagementGroup -GroupName 'Tailspin' -ErrorAction SilentlyContinue) {
                Write-Host -Message "   - Running Remove-AzOpsManagementGroup"
                Remove-AzOpsManagementGroup -GroupName  'Tailspin' -Verbose
            }

            # Task: Initialize azops/
            Write-Host -Message "   - Running Initialize-AzOpsRepository"
            ...
            Get-ChildItem -Path "$PSScriptRoot/../examples/10-create-managementgroup.parameters.json" | ForEach-Object {
                ...
                Write-Host -Message "   - Running New-AzOpsStateDeployment for 10-create-managementgroup.parameters.json"
            ...
            Get-ChildItem -Path "$PSScriptRoot/../examples/20-create-child-managementgroup.parameters.json" | ForEach-Object {
                Copy-Item -Path $_.FullName  -Destination $TestDrive
                $content = Get-Content -Path (Join-Path -Path $TestDrive -ChildPath $_.Name) | ConvertFrom-Json -Depth 100
                $content.parameters.input.value.ParentId = ("/providers/Microsoft.Management/managementGroups/" + (Get-AzTenant).Id)
                $content | ConvertTo-Json -Depth 100 | Out-File -FilePath (Join-Path -Path $TestDrive -ChildPath $_.Name)

                Write-Host -Message "   - Running New-AzOpsStateDeployment for 20-create-child-managementgroup.parameters.json"
                New-AzOpsStateDeployment -FileName (Join-Path -Path $TestDrive -ChildPath $_.Name)
            }

            # Task: Deployment of 30-create-policydefinition-at-managementgroup.parameters.json
            Get-ChildItem -Path "$PSScriptRoot/../examples/30-create-policydefinition-at-managementgroup.parameters.json" | ForEach-Object {
                Copy-Item -Path $_.FullName  -Destination $TestDrive
                $content = Get-Content -Path (Join-Path -Path $TestDrive -ChildPath $_.Name) | ConvertFrom-Json -Depth 100
                $content.parameters.input.value.ParentId = ("/providers/Microsoft.Management/managementGroups/" + (Get-AzTenant).Id)
                $content | ConvertTo-Json -Depth 100 | Out-File -FilePath (Join-Path -Path $TestDrive -ChildPath $_.Name)

                Write-Host -Message "   - Running New-AzOpsStateDeployment for 30-create-policydefinition-at-managementgroup.parameters.json"
                New-AzOpsStateDeployment -FileName (Join-Path -Path $TestDrive -ChildPath $_.Name)
            }

            # State: Disabling this due to bug where Policy assignment fails for first time.
            # Get-ChildItem -Path "$PSScriptRoot/../../examples/40-create-policyassignment-at-managementgroup.parameters.json" | ForEach-Object {
            #     Copy-Item -Path $_.FullName  -Destination $TestDrive
            #     $content = Get-Content -Path (Join-Path -Path $TestDrive -ChildPath $_.Name) | ConvertFrom-Json -Depth 100
            #     $content.parameters.input.value.ParentId = ("/providers/Microsoft.Management/managementGroups/" + (Get-AzTenant).Id)
            #     $content | ConvertTo-Json -Depth 100 | Out-File -FilePath (Join-Path -Path $TestDrive -ChildPath $_.Name)
        ...
        Context "In-a-Box" {
            ...
            It "Passes Policy Definition Test" {
                ...
                $AzOpsReferencePolicyCount = (Get-ChildItem "$PSScriptRoot/../azopsreference/3fc1081d-6105-4e19-b60c-1ec1252cf560/contoso/.AzState/Microsoft.Authorization_policyDefinitions*.json").count
                foreach ($policyDefinition in (Get-ChildItem "$PSScriptRoot/../azopsreference/3fc1081d-6105-4e19-b60c-1ec1252cf560/contoso/.AzState/Microsoft.Authorization_policyDefinitions*.json")) {
                ...
                foreach ($policyDefinition in (Get-ChildItem "$TailspinAzOpsState/Microsoft.Authorization_policyDefinitions*.json")) {
                    #Write-Host -Message "   - Deploying Policy Definition: $policyDefinition"
            ...
            It "Passes PolicySet Definition Test" {
                $TailspinAzOpsState = ((Get-ChildItem -Recurse -Directory -path $env:AzOpsState) | Where-Object  { $_.Name -eq 'Tailspin' }).FullName
                $AzOpsReferencePolicySetCount = (Get-ChildItem "$PSScriptRoot/../azopsreference/3fc1081d-6105-4e19-b60c-1ec1252cf560/contoso/.AzState/Microsoft.Authorization_policySetDefinitions*.json").count
                foreach ($policySetDefinition in (Get-ChildItem "$PSScriptRoot/../azopsreference/3fc1081d-6105-4e19-b60c-1ec1252cf560/contoso/.AzState/Microsoft.Authorization_policySetDefinitions*.json")) {
