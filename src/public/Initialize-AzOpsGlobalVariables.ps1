...
function Initialize-AzOpsGlobalVariables {
    ...
    begin {
        ...
        $AzOpsEnvVariables = @{
            AzOpsState                          = (Join-Path $pwd -ChildPath "azops")   #Folder to store AzOpsState articfacts
            AzOpsMasterTemplate                 = 'src\template.json'                   #Master template json
            AzOpsStateConfig                    = 'src\AzOpsStateConfig.json'           #Configuration file for resource serialization
