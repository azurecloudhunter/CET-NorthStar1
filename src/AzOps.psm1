    ...
    AzOpsScope([System.IO.FileInfo] $path) {
        if (Test-path $path) {
            if ($path.Extension.Equals('.json')) {
                ...
                if ("ResourceId" -in $resourcepath.parameters.input.value) {
                    #Resource
                    $this.InitializeMemberVariables($resourcepath.parameters.input.value.ResourceId)
                }
                elseif ("Id" -in $resourcepath.parameters.input.value) {
...
function New-AzOpsScope {
    ...
    process {
        ...
        elseif ($PSCmdlet.ParameterSetName -eq "pathfile") {
            ...
            if ($path.endswith('.AzState') -or $path.endswith('.AzState/') -or $path.endswith('.AzState\') ) {
                $path = $path.replace('.AzState', '')
            }
