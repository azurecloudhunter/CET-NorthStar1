...
function Test-AzOpsDuplicateSubMgmtGroup {
    ...
    process {
        ...
        $DuplicateNames += $global:AzOpsSubscriptions | Group-Object -Property Name | Where-Object { $_.Count -gt 1 } | ForEach-Object -Process {
            [PSCustomObject]@{
        ...
        $DuplicateNames += $global:AzOpsAzManagementGroup | Group-Object -Property DisplayName | Where-Object { $_.Count -gt 1 } | ForEach-Object -Process {
            [PSCustomObject]@{
                DuplicateName = $_.Name
                Count         = $_.Count
